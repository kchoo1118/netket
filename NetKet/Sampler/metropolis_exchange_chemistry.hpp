...
#ifndef NETKET_METROPOLIS_EXCHANGE_CHEMISTRY_HPP
#define NETKET_METROPOLIS_EXCHANGE_CHEMISTRY_HPP
...
template <class H>
class MetropolisExchangeChemistry : public AbstractSampler {
 ...
 public:
  ...
  void Init() {
    ...
    if (mynode_ == 0) {
      std::cout << "# Metropolis Exchange Chemistry sampler is ready "
                << std::endl;
    }
  }
    ...
    if (randtransitions_) {
      ...
      for (int i = 0; i < std::max(1., 1. / acceptancead_) * sweepsize_; i++) {
        ...
        double ratio = std::norm(
            std::exp(psi_.LogValDiff(v_, tochange_[si], newconfs_[si], lt_)));
