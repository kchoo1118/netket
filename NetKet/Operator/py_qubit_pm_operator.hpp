...
#ifndef NETKET_PYQUBITPMOPERATOR_HPP
#define NETKET_PYQUBITPMOPERATOR_HPP
...
void AddQubitPMOperator(py::module &subm) {
  py::class_<QubitPMOperator, AbstractOperator>(
      subm, "QubitPMOperator", R"EOF(A custom qubit operator.)EOF")
      .def(py::init([](const AbstractHilbert &hi, std::vector<std::string> ops,
                       std::vector<std::complex<double>> opweights) {
             return QubitPMOperator{hi, std::move(ops), std::move(opweights)};
           }),
