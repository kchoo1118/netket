...
#ifndef NETKET_SYMMCONVSQUARELAYER_HH
#define NETKET_SYMMCONVSQUARELAYER_HH
...
class SymmSquareConvolutional : public AbstractLayer<T> {
  ...
  MatrixType kernels_;      // kernel parameters
  VectorType bias_;         // Bias parameters, b(out_size x 1)

  int symm_kernel_size_;
  MatrixType kernelsymm_;       // Symmetric kernel parameters
  Eigen::MatrixXd dermatsymm_;  // To map bare derivatives to symm derivative

  std::string name_;

  std::vector<std::vector<int>> kauto_;
 ...
 public:
  ...
  explicit SymmSquareConvolutional(const json &pars) : activation_() {
  ...
  void Init() {
    ...
    symm_kernel_size_ = (kernel_length_ / 2) * (kernel_length_ / 2) +
                        (kernel_length_ % 2) * ((kernel_length_ / 2) + 1);
    in_size_ = in_channels_ * nv_;
    out_size_ = out_channels_ * nv_;
    ...
    // Construct kauto_
    kauto_.resize(symm_kernel_size_);
    Eigen::MatrixXi rot(2, 2);
    rot << 0, -1, 1, 0;
    if (kernel_length_ % 2 == 0) {
      Eigen::VectorXi offset(2);
      offset.setConstant(kernel_length_);
      Eigen::VectorXi shift(2);
      shift.setConstant(1);
      int kk = 0;
      for (int j = 0; j < kernel_length_ / 2; ++j) {
        for (int i = 0; i < kernel_length_ / 2; ++i) {
          ...
          for (int c = 0; c < 4; ++c) {
            kauto_[kk].push_back(
                Coord2Site((coord - shift + offset) / 2, kernel_length_));
            coord = rot * coord;
          }
          ++kk;
        }
      ...
      Eigen::VectorXi offset(2);
      offset.setConstant(kernel_length_ / 2);
      kauto_[0].push_back(int(kernel_size_ / 2));
      int kk = 1;
      for (int j = 0; j < kernel_length_ / 2; ++j) {
        for (int i = 0; i < kernel_length_ / 2 + 1; ++i) {
          Eigen::VectorXi coord(2);
          coord(0) = i;
          coord(1) = j + 1;
          for (int c = 0; c < 4; ++c) {
            kauto_[kk].push_back(Coord2Site(coord + offset, kernel_length_));
            coord = rot * coord;
          }
          ++kk;
        }
    ...
    dermatsymm_.setZero();
    for (int i = 0; i < symm_kernel_size_; ++i) {
      for (auto j : kauto_[i]) {
        dermatsymm_(i, j) = 1;
      }
    }
    ...
    for (int i = 0; i < nv_; ++i) {
      ...
      for (auto t : trans) {
        ...
        flippedneigh.push_back(Coord2Site(newcoord, l_));
      }
    ...
    kernelsymm_.resize(in_channels_ * symm_kernel_size_, out_channels_);
    ...
    npar_ = in_channels_ * symm_kernel_size_ * out_channels_;
    ...
    InfoMessage(buffer) << "Symm Square Convolutional Layer: " << in_size_
                        << " --> " << out_size_ << std::endl;
  ...
  void GetParameters(VectorType &pars, int start_idx) const override {
    ...
    for (int j = 0; j < out_channels_; ++j) {
      for (int i = 0; i < in_channels_ * symm_kernel_size_; ++i) {
        pars(k) = kernelsymm_(i, j);
        ++k;
      }
    }
  }
  ...
  void SetBareParameters() {
    for (int j = 0; j < out_channels_; ++j) {
      for (int i = 0; i < in_channels_; ++i) {
        for (int k = 0; k < symm_kernel_size_; ++k) {
          for (auto n : kauto_[k]) {
            kernels_(i * kernel_size_ + n, j) =
                kernelsymm_(i * symm_kernel_size_ + k, j);
          }
        }
      }
    }
  ...
  void Backprop(const VectorType &prev_layer_output,
    ...
    Eigen::Map<MatrixType> der_w(
        der.data() + kd, in_channels_ * symm_kernel_size_, out_channels_);
    for (int i = 0; i < in_channels_; ++i) {
      for (int j = 0; j < out_channels_; ++j) {
        der_w.block(i * symm_kernel_size_, j, symm_kernel_size_, 1) =
            dermatsymm_ *
            lowered_image2_.block(0, i * kernel_size_, nv_, kernel_size_)
                .transpose() *
            dLz_reshaped.block(0, j, nv_, 1);
      }
    }
