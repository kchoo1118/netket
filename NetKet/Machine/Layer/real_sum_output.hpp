...
#ifndef NETKET_REALSUMOUTPUT_HH
#define NETKET_REALSUMOUTPUT_HH
...
template <typename T>
class RealSumOutput : public AbstractLayer<T> {
 ...
 public:
  ...
  /// Constructor
  explicit RealSumOutput(const json &pars) : I(0.0, 1.0) {
  ...
  inline void LinearTransformation(const VectorType &input, LookupType &theta) {
    theta[0](0) = input.segment(0, in_size_ / 2).sum() +
                  I * input.segment(in_size_ / 2, in_size_ / 2).sum();
  }
  ...
  inline void UpdateTheta(const VectorType &prev_input,
    ...
    for (int s = 0; s < num_of_changes; s++) {
      ...
      if (sf < in_size_ / 2) {
        ...
        theta[0](0) += I * ((newconf[s] - prev_input(sf)));
      }
    }
  }
  ...
  void Backprop(const VectorType & /*prev_layer_output*/,
    ...
    din.resize(in_size_);
    din.segment(0, in_size_ / 2).setConstant(dout(0));
    din.segment(in_size_ / 2, in_size_ / 2).setConstant(I * dout(0));
  }
