...
#ifndef NETKET_REALINPUTFULLCONNLAYER_HH
#define NETKET_REALINPUTFULLCONNLAYER_HH
...
class RealInputFullyConnected : public AbstractLayer<T> {
  ...
  Eigen::VectorXd bias_;    // Bias parameters, b(out_size x 1)
 ...
 public:
  ...
  /// Constructor
  RealInputFullyConnected(const int input_size, const int output_size,
  ...
  explicit RealInputFullyConnected(const json &pars) : activation_() {
    Init(pars);
  }
  ...
  void Init(const json &pars) {
    ...
    InfoMessage(buffer) << "Real Input Fully Connected Layer " << in_size_
                        << " --> " << out_size_ << std::endl;
  ...
  void GetParameters(VectorType &pars, int start_idx) const override {
    ...
    pars.segment(k, in_size_ * out_size_) =
        Eigen::Map<const Eigen::VectorXd>(weight_.data(), in_size_ * out_size_);
  }
  ...
  void SetParameters(const VectorType &pars, int start_idx) override {
    ...
    Eigen::Map<Eigen::VectorXd> w{weight_.data(), in_size_ * out_size_};

    w = pars.segment(k, in_size_ * out_size_).real();
  }
  ...
  // Updates theta given the input v, the change in the input (input_changes
  // and prev_input)
