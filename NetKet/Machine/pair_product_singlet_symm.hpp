...
#ifndef NETKET_PAIRPRODUCTSINGLETSYMM_HPP
#define NETKET_PAIRPRODUCTSINGLETSYMM_HPP

namespace netket {

// PairProduct machine class for spin 1/2 degrees.
class PairProductSingletSymm : public AbstractMachine {
 ...
 public:
  explicit PairProductSingletSymm(const AbstractHilbert &hilbert)
  ...
  void Init() {
    ...
    Ftemp_ = Eigen::MatrixXi::Zero(nv_, nv_);
    ...
    for (int i = 0; i < nv_; i++) {
      for (int j = 0; j < nv_; j++) {
        if (params.count(Ftemp_(i, j))) {
          ...
          std::cerr << "Error in PairProductSingletSymm: Ftemp_" << std::endl;
          std::abort();
        }
    ...
    for (int i = 0; i < nv_; i++) {
      for (int j = 0; j < nv_; j++) {
        ...
        if (ksymm < 1 || ksymm - 1 >= npar_) {
          std::cerr << "Error in PairProductSingletSymm: DerMatSymm_"
                    << std::endl;
          std::abort();
        }
