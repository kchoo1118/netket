...
#ifndef NETKET_PYAUTOREGRESSIVEREAL_HPP
#define NETKET_PYAUTOREGRESSIVEREAL_HPP
...
void AddAutoregressiveRealMachine(py::module &subm) {
  py::class_<AutoregressiveRealMachine, AbstractMachine>(
      subm, "AutoregressiveRealMachine", R"EOF(
          A fully connected Restricted Boltzmann Machine (AutoregressiveMachine).
          )EOF")
      .def(py::init<const AbstractHilbert &, int, int, bool, bool, int,
                    double>(),
           py::keep_alive<1, 2>(), py::arg("hilbert"), py::arg("n_hidden") = 0,
           py::arg("alpha") = 0, py::arg("use_hidden_bias") = true,
           py::arg("use_output_bias") = true, py::arg("total") = -1,
           py::arg("offset") = 0.0,
           R"EOF(
                   Constructs a new ``AutoregressiveRealMachine`` machine:
