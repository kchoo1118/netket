...
#ifndef NETKET_LOGPSISUM_HPP
#define NETKET_LOGPSISUM_HPP
...
template <class T>
class LogPsiSum : public AbstractMachine<T> {
 ...
 public:
  ...
  explicit LogPsiSum(const Graph &graph, const Hamiltonian &hamiltonian,
                     const json &pars)
      : ham_(hamiltonian),
    ...
    InfoMessage() << "Using Log Sum of 2 wavefunctions" << std::endl;
  }
  ...
  int Npar() const override { return psi1_->Npar(); }
  ...
  VectorType GetParameters() override { return psi1_->GetParameters(); }

  void SetParameters(const VectorType &pars) override {
    psi1_->SetParameters(pars);
  }

  // Value of the logarithm of the wave-function
  T LogVal(const Eigen::VectorXd &v) override {
    return psi1_->LogVal(v) + psi2_->LogVal(v);
  }
  ...
  void to_json(json &j) const override { psi1_->to_json(j); }

  void from_json(const json &j) override { psi1_->from_json(j); }
