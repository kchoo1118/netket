...
class PsiSum : public AbstractMachine<T> {
 ...
 public:
  ...
  explicit PsiSum(const Graph &graph, const Hamiltonian &hamiltonian,
                  const json &pars)
      : ham_(hamiltonian), hilbert_(hamiltonian.GetHilbert()), graph_(graph) {
    nv_ = hilbert_.Size();
    alpha_.resize(2);
    alpha_.setConstant(1.0);

    InfoMessage() << "Machine 1 is ";
    psi1_ = Init(graph_, hilbert_, pars["SumMachine"][0]);

    InfoMessage() << "Machine 2 is ";
    psi2_ = Init(graph_, hilbert_, pars["SumMachine"][1]);
  ...
  void InitParameters(const json &pars) {
    psi1_->InitRandomPars(pars["SumMachine"][0]);

    if (FieldExists(pars["SumMachine"][0]["Machine"], "InitFile")) {
      std::string filename = pars["SumMachine"][0]["Machine"]["InitFile"];
    ...
    psi2_->InitRandomPars(pars["SumMachine"][1]);

    if (FieldExists(pars["SumMachine"][1]["Machine"], "InitFile")) {
      std::string filename = pars["SumMachine"][1]["Machine"]["InitFile"];
