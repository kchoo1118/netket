...
class FFNN : public AbstractMachine<T> {
 ...
 public:
  ...
  void Init(const json &pars) {
    ...
    InfoMessage(buffer) << "# FFNN Real Initizialized with " << nlayer_
                        << " Layers: ";
  ...
  VectorType LogValDiff(
    ...
    for (int k = 0; k < nconn; ++k) {
      ...
      if (tochange[k].size() != 0) {
        ...
        logvaldiffs(k) += LogVal(v, ltnew).real() - current_val.real();
      }
    }
    return logvaldiffs;
  }
  ...
  T LogValDiff(const Eigen::VectorXd &v, const std::vector<int> &tochange,
    ...
    if (tochange.size() != 0) {
      ...
      return LogVal(v, ltnew).real() - LogVal(v, lt).real();
    } else {
      return 0.0;
    }
  }
